...
export default function AibaptFAQ() {
  ...
  return (
    <section className="w-full py-24 bg-[#F4F1EB] px-4 sm:px-6 lg:px-8">
      <div className="max-w-[1000px] mx-auto">
        <h2 className="text-4xl md:text-[56px] font-serif text-[#261C4F] leading-[1.1] mb-12">
        ...
        <div className="flex flex-col gap-4">
          {faqs.map((faq, index) => {
            ...
            return (
              <div 
                key={index} 
                className={`w-full rounded-[24px] overflow-hidden transition-all duration-300 cursor-pointer ${isActive ? 'bg-[#F3E8B5]' : 'bg-white hover:bg-white/90'}`}
                onClick={() => toggleFAQ(index)}
              >
                <div className="px-8 py-6 md:py-8 flex justify-between items-center">
                  <h3 className="text-[22px] md:text-[26px] font-serif text-[#261C4F] pr-4">
                    {faq.question}
                  </h3>
                  <div className="shrink-0 text-[#261C4F]">
                    {isActive ? (
