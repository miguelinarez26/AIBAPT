...
export default function AibaptEvents() {
  ...
  return (
    <section className="w-full py-24 bg-[#F4F1EB] px-4 sm:px-6 lg:px-8 border-t border-[#e5e5e5]">
      <div className="max-w-[1280px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
           <div className="max-w-2xl">
             <p className="text-[#C89B7B] text-[13px] font-semibold tracking-[0.2em] uppercase mb-4">Desarrollo Profesional</p>
             <h2 className="text-4xl md:text-[56px] font-serif text-[#261C4F] leading-[1.1] mb-4">
               Próximos Eventos <span className="italic font-light text-[#0F75A1]">2026</span>
             </h2>
             <p className="text-[#555555] text-lg">Entrenamientos intensivos y certificaciones internacionales para psicoterapeutas bajo el estándar de AIBAPT.</p>
           </div>
           <Link href="/eventos" className="inline-flex bg-transparent border border-[#0F75A1] text-[#0F75A1] px-8 py-3.5 rounded-full font-medium hover:bg-[#0F75A1] hover:text-white transition-all text-[15px]">
             ...
             <div key={i} className="group cursor-pointer">
                <div className="w-full h-[320px] rounded-2xl overflow-hidden mb-6 relative shadow-sm">
                   ...
                   <div className="absolute top-5 left-5 bg-white/95 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm">
                      <span className="text-[#0F75A1] text-[11px] font-bold tracking-[0.15em] uppercase">{ev.type}</span>
                   </div>
                </div>
                <h3 className="text-[26px] font-serif text-[#261C4F] group-hover:text-[#0F75A1] transition-colors leading-[1.3] mb-4 pr-4">{ev.title}</h3>
                <div className="flex items-center text-[#C89B7B] font-semibold text-sm group-hover:gap-2 transition-all tracking-wider uppercase">
