  ...
  return (
    <section className="w-full bg-background-light py-24 px-4 sm:px-8 lg:px-[140px] flex flex-col lg:flex-row gap-16 xl:gap-32 items-center lg:items-start justify-center">
      ...
      <div className="w-full max-w-[420px] bg-white rounded-[16px] shadow-[0_20px_50px_rgba(0,0,0,0.06)] overflow-hidden shrink-0">
        ...
        {/* Days Header */}
        <div className="grid grid-cols-7 bg-secondary text-white text-[11px] font-semibold tracking-wider py-3">
        ...
        <div className="grid grid-cols-7 text-[15px] text-center">
          {Array.from({ length: 35 }).map((_, i) => {
            ...
            return (
              <div 
                key={i} 
                className={`h-[65px] flex items-center justify-center relative ${isGreyCol ? 'bg-gray-50' : 'bg-white'} ${isValidDate ? 'text-text-dark' : 'text-transparent pointer-events-none'}`}
              >
                {isEvent ? (
                  <button 
                    onClick={() => setSelectedDate(date)}
                    className={`w-11 h-11 rounded-full transition-all duration-300 flex items-center justify-center group ${isSelected ? 'bg-accent text-white shadow-md scale-105' : 'hover:bg-highlight text-text-light'}`}
                  >
                    <span>{date}</span>
                    {/* Tooltip */}
                    <div className="absolute -top-10 bg-white shadow-lg text-text-light text-xs font-medium py-1.5 px-3 rounded opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 whitespace-nowrap z-10 pointer-events-none">
                  ...
                  <div className={`w-11 h-11 flex items-center justify-center rounded-full ${isCurrentDay ? 'border-2 border-primary text-primary font-medium' : ''}`}>
                    {date}
                  </div>
                ) : (
      ...
      <div className="flex-1 max-w-[540px]">
        <p className="text-accent text-[13px] font-semibold tracking-[0.2em] uppercase mb-4">Webinar Details</p>
        ...
        {selectedEvent ? (
          <div key={selectedDate} className="animate-fade-in-up">
            ...
            <button className="bg-accent text-white px-8 py-3.5 rounded-full font-medium hover:bg-accent-light transition-colors shadow-sm text-[15px]">
              Register Now
            </button>
          </div>
        ) : (
          <div>
            <h2 className="text-4xl md:text-[44px] font-serif text-text-light mb-6 leading-tight">Select a date</h2>
            <p className="text-text-dark text-lg leading-relaxed">Click on a highlighted date in the calendar to view webinar details and register for our upcoming online sessions.</p>
          </div>
        )}

        <div className="mt-14">
          <Link href="/webinars" className="text-primary font-medium hover:text-secondary flex items-center gap-2 transition-colors w-fit group">
