...
function ResetPasswordContent() {
    ...
    const handleResetRequest = async (e: React.FormEvent) => {
        ...
        const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
            redirectTo: `${window.location.origin}/${lang}/reset-password/update`,
        });
    ...
    return (
        <div className="pt-20 min-h-screen bg-accent/10 dark:bg-background-dark flex items-center justify-center p-6 relative overflow-hidden">
            ...
            <div className="w-full max-w-md bg-white dark:bg-surface-dark rounded-3xl shadow-xl p-8 md:p-10 relative z-10 border border-accent/20 dark:border-gray-800">
                <Link href={`/${lang}/login`} className="flex items-center gap-2 text-sm font-bold text-text-muted hover:text-primary mb-8 transition-colors">
                    <ArrowLeft className="w-4 h-4" />
                    {lang === 'es' ? 'Volver al Login' : 'Voltar ao Login'}
                </Link>

                <div className="mb-10 text-center">
                    <h1 className="text-3xl font-black font-display text-secondary dark:text-white mb-2">{t("auth.reset_title")}</h1>
                    <p className="text-sm text-text-muted dark:text-white/60">{t("auth.reset_desc")}</p>
                </div>
                ...
                {success ? (
                    <div className="text-center space-y-6 animate-fade-in">
                        ...
                        <Link 
                            href={`/${lang}/login`}
                            className="block w-full py-3.5 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark transition-all shadow-md"
                        >
                    ...
                    <form onSubmit={handleResetRequest} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold uppercase tracking-wider text-text-muted dark:text-white/60" htmlFor="email">
                                {lang === 'es' ? 'Tu Email Registrado' : 'Seu Email Registrado'}
                            </label>
                        ...
                        >
                            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : t("auth.reset_btn")}
                        </button>
                    </form>
                )}
            </div>
